...
import { CheckCircle2, ChevronRight, CreditCard, Landmark, Banknote, CalendarDays, Wallet, FileText, Receipt } from "lucide-react";
...
function Step1Review({ payroll, totalDeductions, net }) {
    return (
            ...
            <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                <div className="px-6 py-4 bg-slate-50 border-b border-slate-200">
                    <h4 className="font-bold text-slate-700 flex items-center gap-2">
                        <FileText className="w-4 h-4" /> Salary Breakdown
                    </h4>
                </div>
                <div className="p-6 space-y-4">
                    ...
                    <div className="flex justify-between items-center text-slate-600">
                        ...
                        <span className="font-medium text-emerald-600">
                            +₹{((payroll?.earnings?.hra || 0) + (payroll?.earnings?.bonus || 0)).toLocaleString()}
                        </span>
                    </div>
                    <div className="flex justify-between items-center text-slate-600">
...
function Step2Details({ paymentDate, setPaymentDate, paymentNote, setPaymentNote, net }) {
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
...
function Step3Method({ paymentMethod, setPaymentMethod }) {
    const methods = [
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="flex-1">
                                <div className="flex justify-between items-center">
                                    <h4 className={`font-bold ${isSelected ? "text-blue-900" : "text-slate-700"}`}>{method.id}</h4>
                                    {method.badge && (
                                        <span className="text-[10px] uppercase tracking-wider font-extrabold bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">
                                            {method.badge}
                                        </span>
                                    )}
                                </div>
                                <p className={`text-sm mt-0.5 ${isSelected ? "text-blue-700/80" : "text-slate-500"}`}>{method.desc}</p>
                            </div>

                            {isSelected && method.id === "Bank Transfer" && (
...
function Step4Processing({ processStatus }) {
    return (
        <div className="py-12 flex flex-col items-center justify-center text-center animate-in fade-in zoom-in-95 duration-500">
            ...
            <style>{`@keyframes progress { 0% { width: 0%; } 20% { width: 30%; } 50% { width: 60%; } 80% { width: 85%; } 100% { width: 100%; } }`}</style>
        </div>
    );
}

function Step5Confirmation({ paymentDate, paymentMethod, net, payroll, onSuccess, onClose }) {
    return (
...
export default function PaymentStepperModal({ payroll, onClose, onSuccess, apiPath }) {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            if (index < statuses.length) {
                setProcessStatus(statuses[index]);
                index += 1;
                return;
            }
    ...
    const executePayment = async () => {
        try {
            const endpoint = apiPath ? `${apiPath}/${payroll._id}/mark-paid` : `/payroll/${payroll._id}/mark-paid`;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-2xl rounded-[2rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                {currentStep < 4 && (
                    <div className="px-8 py-4 bg-slate-50/50 border-b border-slate-100 flex justify-between shrink-0">
                        {STEPS.map((step, index) => (
                            <div key={step.id} className={`flex flex-col items-center gap-2 relative z-10 w-full ${index === STEPS.length - 1 ? "max-w-[40px]" : ""}`}>
                                ...
                                <span className={`text-[10px] font-bold uppercase tracking-wide absolute -bottom-5 w-max ${currentStep >= step.id ? "text-slate-700" : "text-slate-400"}`}>
                                    {step.title}
                                </span>

                                {index < STEPS.length - 1 && (
                                    <div className={`absolute top-4 left-8 right-[-1rem] h-[2px] -z-10 ${currentStep > step.id ? "bg-emerald-500" : "bg-slate-200"}`}></div>
                                )}
                            </div>
                        ))}
                    </div>
                ...
                <div className="p-8 overflow-y-auto flex-1 min-h-[300px]">
                    {currentStep === 1 && <Step1Review payroll={payroll} totalDeductions={totalDeductions} net={net} />}
                    {currentStep === 2 && <Step2Details paymentDate={paymentDate} setPaymentDate={setPaymentDate} paymentNote={paymentNote} setPaymentNote={setPaymentNote} net={net} />}
                    {currentStep === 3 && <Step3Method paymentMethod={paymentMethod} setPaymentMethod={setPaymentMethod} />}
                    {currentStep === 4 && <Step4Processing processStatus={processStatus} />}
                    {currentStep === 5 && <Step5Confirmation paymentDate={paymentDate} paymentMethod={paymentMethod} net={net} payroll={payroll} onSuccess={onSuccess} onClose={onClose} />}
                </div>
                ...
                {currentStep < 4 && (
                    <div className="px-8 py-6 border-t border-slate-100 bg-slate-50 flex justify-between shrink-0 rounded-b-[2rem]">
                        <button
                            ...
                            className={`px-6 py-2.5 rounded-xl font-bold transition-colors ${currentStep === 1 ? "text-transparent cursor-default" : "text-slate-500 hover:bg-slate-200 bg-slate-100"}`}
                        >
                            Back
                        </button>
